...
document.addEventListener('DOMContentLoaded', function() {
    ...
    calculateTssRangeBtn.addEventListener('click', function() {
        ...
        // Show Step 3 section and load workout options
        loadWorkoutOptions();
    });
    ...
    if (calculateBtn) {
        calculateBtn.addEventListener('click', async function() {
            ...
            // Get athlete ID from window object (set by openTSSPlanner function)
            const athleteId = window.currentTSSAthleteId || null;
            ...
            console.log("🚀 TSS Planner PAYLOAD ====>", payload);
            
            try {
                if (typeof showLoader === 'function') showLoader();
                
                const response = await fetch('/api/training-stress-recommendation', {
                    method: 'POST',
    ...
    // ============================================================================
    // STEP 3: Post Workouts - Google Sheets Integration
    // ============================================================================
    
    const postWorkoutsSection = document.getElementById('postWorkoutsSection');
    const tssValueSelect = document.getElementById('tss_value_select');
    const frequencySelect = document.getElementById('frequency_select');
    const intensitySelect = document.getElementById('intensity_select');
    const workoutDate = document.getElementById('workout_date');
    const postWorkoutBtn = document.getElementById('postWorkoutBtn');
    const workoutPostResponse = document.getElementById('workoutPostResponse');
    const currentBlockType = document.getElementById('current_block_type');
    const currentSportType = document.getElementById('current_sport_type');
    
    let workoutOptionsData = null;
    
    /**
     * Load workout options from Google Sheets
     */
    async function loadWorkoutOptions() {
        if (!currentApiResponse) {
            console.log('⚠️ No API response available for workout options');
            return;
        }
        
        const selectedSport = getSelectedSport();
        const blockTypeValue = document.getElementById('block_type').value;
        
        if (!blockTypeValue) {
            console.log('⚠️ No block type selected');
            return;
        }
        
        console.log(`📊 Loading workout options for ${selectedSport}/${blockTypeValue}`);
        
        try {
            const response = await fetch('/api/fetch-tss-workout-options', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    sport_type: selectedSport,
            ...
            const data = await response.json();
            console.log('✅ Workout options loaded:', data);
            
            if (!response.ok) {
                throw new Error(data.error || 'Failed to load workout options');
            }
            
            workoutOptionsData = data.data;
            
            // Populate TSS value dropdown
            tssValueSelect.innerHTML = '<option value="">Select TSS</option>';
            workoutOptionsData.forEach(option => {
                const opt = document.createElement('option');
                opt.value = option.tss;
                opt.textContent = `${option.tss} TSS`;
                tssValueSelect.appendChild(opt);
            });
            
            // Update display
            currentBlockType.textContent = blockTypeValue.replace('_', '/').toUpperCase();
            currentSportType.textContent = selectedSport.toUpperCase();
            
            // Set default workout date to next Monday
            const nextMonday = getNextMonday();
            workoutDate.value = nextMonday.toISOString().split('T')[0];
            
            // Show Step 3 section
            postWorkoutsSection.style.display = 'block';
            
        } catch (error) {
            console.error('❌ Error loading workout options:', error);
            alert('Error loading workout options: ' + error.message);
        }
    }
    ...
    /**
     * Handle TSS value selection
     */
    tssValueSelect.addEventListener('change', function() {
        const tssValue = parseInt(this.value);
        
        if (!tssValue || !workoutOptionsData) {
        ...
        const selectedOption = workoutOptionsData.find(opt => opt.tss === tssValue);
        
        if (!selectedOption) return;
        
        // Populate frequency dropdown
        frequencySelect.innerHTML = '<option value="">Select Frequency</option>';
        selectedOption.frequency.forEach(freq => {
            const opt = document.createElement('option');
            opt.value = freq;
            opt.textContent = `${freq}x per week`;
            frequencySelect.appendChild(opt);
        });
        
        frequencySelect.disabled = false;
        console.log(`📊 TSS ${tssValue} selected, ${selectedOption.frequency.length} frequency options available`);
    });
    ...
    frequencySelect.addEventListener('change', function() {
        const tssValue = parseInt(tssValueSelect.value);
        ...
        intensitySelect.innerHTML = '<option value="">Select Intensity</option>';
        selectedOption.intensity.forEach(intensity => {
            const opt = document.createElement('option');
            opt.value = intensity;
        ...
        console.log(`📊 Frequency ${this.value} selected, ${selectedOption.intensity.length} intensity options available`);
    });
    
    /**
     * Post workout week to TrainingPeaks
     */
    postWorkoutBtn.addEventListener('click', async function() {
        const athleteId = window.currentTSSAthleteId || null;
        const selectedSport = getSelectedSport();
        const blockTypeValue = document.getElementById('block_type').value;
        const tssValue = parseInt(tssValueSelect.value);
        ...
        const startDateValue = workoutDate.value;
        ...
        if (!tssValue || !frequency || !intensity || !startDateValue) {
            alert('Please select all workout parameters');
            return;
        }
        
        if (!confirm(`Post ${frequency}x ${selectedSport} workouts (${tssValue} TSS, ${intensity}) starting ${startDateValue} to athlete ${athleteId}?`)) {
            return;
        }
        
        postWorkoutBtn.disabled = true;
        workoutPostResponse.style.display = 'none';
        
        try {
            console.log('📅 Posting workout week...', {
                athleteId,
                selectedSport,
                blockTypeValue,
                tssValue,
            ...
            const response = await fetch('/api/post-workout-week', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            const data = await response.json();
            console.log('✅ Workout week response:', data);
            
            if (!response.ok) {
                throw new Error(data.error || 'Failed to post workouts');
            }
            
            // Display success message
            workoutPostResponse.innerHTML = `
                <div class="alert alert-success">
                    <strong>✅ Success!</strong>
                    <p class="mb-2">${data.message || 'Workout week posted successfully'}</p>
                    ${data.workouts ? `
                        <p class="mb-1"><strong>Workouts Posted:</strong></p>
                        <ul class="mb-0">
                            ${data.workouts.map(w => `<li>${w.day}: ${w.duration}min ${w.workout_type} (${w.tss} TSS)</li>`).join('')}
                        </ul>
                    ` : ''}
                    ${data.note ? `<p class="mt-2 mb-0 text-muted small">${data.note}</p>` : ''}
                </div>
            `;
            workoutPostResponse.style.display = 'block';
            
        } catch (error) {
            console.error('❌ Error posting workout week:', error);
